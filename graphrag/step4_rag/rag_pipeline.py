...
PROMPT_DETAIL = RagTemplate(
    template="""당신은 사내 기술 문서를 분석하여 실용적인 안내를 제공하는 어시스턴트입니다.
검색된 문서의 내용을 빠짐없이 상세하게 안내하세요.
...
검색 결과의 모든 내용을 빠짐없이 포함하여 작성하세요:
- 단계별 절차가 있으면 번호를 매겨 구체적으로 안내
  - 각 단계의 하위 설명, 옵션, 주의사항도 포함
- 관련 팁이나 참고사항이 있으면 별도로 안내
- 요약하지 말고 검색된 내용 전체를 활용하세요
...
class RAGPipeline:
    ...
    def build(self) -> tuple:
        ...
        Returns:
            (retriever, graphrag_list, graphrag_summary, graphrag_overview, graphrag_detail, llm)
        """
        ...
        graphrag_detail = GraphRAG(
            llm=self.llm,
            retriever=retriever,
            prompt_template=PROMPT_DETAIL,
        )

        return retriever, graphrag_list, graphrag_summary, graphrag_overview, graphrag_detail, self.llm
